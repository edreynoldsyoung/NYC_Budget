---
title: "New York City Budget - per capita (real dollars)"
author: "Ed Young"
...
fetch_acs_pop <- function(year) {
  ...
  # Census API returns a JSON array where row 1 is headers and row 2 is data.
  # Parse defensively so we do not depend on matrix dimnames.
  header <- unlist(dat[[1]], use.names = FALSE)
  values <- unlist(dat[[2]], use.names = FALSE)
  row <- stats::setNames(as.list(values), header)

  tibble(
    year = year,
    population = as.numeric(row[["B01003_001E"]])
  )
}
...
acs_years <- setdiff(acs_years, 2020)
